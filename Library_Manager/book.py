...
class Book:
    def __init__(self, title, author, isbn, year, publisher, genre, pages, format, price, 
    ...
    def format(self, format):
        if format not in ["Paperback", "Hardcover", "E-book", "Audiobook"]:
            raise ValueError("Invalid Format. Allowed formats: Paperback, Hardcover, E-book, Audiobook")
        self._format = format
    ...
    def status (self, status):
        if status not in ["Read", "Want to read", "In progress", "Unread"]:
            raise ValueError ("Invalid read status. Allowed statuses: Read, Want to read, In progress, Unread")
        self._status = status
