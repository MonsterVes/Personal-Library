...
def add_book():
    ...
    while True:
        try:
            ...
            year_obj = pyip.inputDate(
                "Enter publishing year (YYYY-MM-DD): ", formats=["%Y-%m-%d"]
            )
            ...
            format = pyip.inputChoice(
                ["Paperback", "Hardcover", "E-book", "Audiobook"],
                prompt="Enter book format (Paperback, Hardcover, E-book, Audiobook): ",
            ).capitalize()
            price = pyip.inputFloat("Enter book price: ", min=0.1)
            purchase_date_obj = pyip.inputDate(
                "Enter date of purchase (YYYY-MM-DD): ", formats=["%Y-%m-%d"]
            )
            ...
            if purchase_date_obj > year_obj:
                ...
                print(
                    "Invalid purchase date. Purchase date is prior to Publishing year. Please re-enter book info."
                )
                continue

            personal_rating = pyip.inputChoice(
                ["1", "2", "3", "4", "5"], prompt="Enter personal rating (1-5): "
            )
            status = pyip.inputChoice(
                ["Read", "Want to read", "In progress", "Unread"],
                prompt="Enter read status (Read, Want to read, In progress, Unread): ",
            ).capitalize()
